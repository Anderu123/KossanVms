...
namespace KossanVMS.Utility
{
    ...
    {
        public bool GenerateGatePass(VisitRecord visitRecord)
        {
            

            var doc = BuildDocument(visitRecord);
        ...
        Document BuildDocument(VisitRecord visitRecord)
        {
         ...
         //  mainTable.AddColumn(Unit.FromMillimeter(60));  // Right column
            ...
            tagHeader.AddFormattedText(visitRecord.VisitRecordTagNo + "\n", TextFormat.Underline);
            ...
            mainHeader.AddFormattedText(visitRecord.VisitRecordBranch.BranchName ?? "Kossan" + "\n", TextFormat.Bold);
            ...
            gatePassNo.AddFormattedText(visitRecord.VisitRecordGatePass + "\n", TextFormat.Underline);
            ...
            AddDetailRow(leftColumn, "COMPANY", visitRecord.VisitRecordContact?.ContactCompany ?? "-");
            AddDetailRow(leftColumn, "VISITOR'S NAME", visitRecord.VisitRecordVisitor?.VisitorFullName ?? "-");
            AddDetailRow(leftColumn, "ID. NO.", visitRecord.VisitRecordVisitor?.VisitorIdNo ?? "-");
            AddDetailRow(leftColumn, "PERSON TO VISIT", visitRecord.VisitRecordVisitPerson ?? "-");
            AddDetailRow(leftColumn, "PURPOSE OF VISIT", visitRecord.VisitRecordPurpose?.PurposeName ??"-");
            ...
            spaceRowTop.TopPadding = Unit.FromMillimeter(2);
            spaceRowTop.BottomPadding = Unit.FromMillimeter(2);
            AddDetailRow(rightColumn, "DATE", visitRecord.VisitRecordInTime.ToString("M/d/yyyy"));
            AddDetailRow(rightColumn, "TIME IN", visitRecord.VisitRecordInTime.ToString("hh.mm.ssTT").ToLower());
            AddDetailRow(rightColumn, "TIME OUT", visitRecord.VisitRecordOutTime?.ToString("hh.mm.ssTT").ToLower() ?? "-");
            AddDetailRow(rightColumn, "VEHICLE NO.", visitRecord.VisitRecordVehicleNo ?? "-");
            AddDetailRow(rightColumn, "D/O NO.", visitRecord.VisitRecordDONo ?? "-");
            AddDetailRow(rightColumn, "TEL NO.", visitRecord.VisitRecordContact?.ContactTel ?? "-");
        ...
        public bool TestGenerateGatePass()
        {
            ...
            var doc = TestBuildDocument(model);
            var renderer = new PdfDocumentRenderer(true);
            renderer.Document = doc;
            renderer.RenderDocument();
        ...
        Document TestBuildDocument(Gatepass m)
        {
            var doc = new Document();
            var style = doc.Styles["Normal"];
            style.Font.Name = "Arial";
